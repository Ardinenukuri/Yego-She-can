...
const MentorService_1 = require("./MentorService");
const DashboardService_1 = require("./DashboardService");
exports.ReportService = {
    generateMentorReportPDF: (mentorId, mentorName) => __awaiter(void 0, void 0, void 0, function* () {
        const dashboardData = yield MentorService_1.MentorService.getDashboardData(mentorId);
        ...
        const htmlContent = `
            ...
            <body>
                ...
                <div class="section">
                    ...
                    <tbody>${courses.map((c) => `<tr><td>${c.title}</td><td>${c.level}</td><td>${c.duration}</td><td>${c.chapters}</td><td>${c.studentsEnrolled}</td></tr>`).join('')}</tbody></table>
                </div>
                <div class="section">
                    ...
                    <tbody>${quizzes.map((q) => `<tr><td>${q.title}</td><td>${q.course}</td><td>${q.expected}</td><td>${q.attempted}</td><td class="passed">${q.passed}</td><td class="failed">${q.failed}</td></tr>`).join('')}</tbody></table>
                </div>
                <div class="section">
                    <h2>Booked Meetings (${bookings.length})</h2>
                    <table><thead><tr><th>Student</th><th>Course</th><th>Time Slot</th><th>Topic</th></tr></thead>
                    <tbody>${bookings.map((b) => `<tr><td>${b.student}</td><td>${b.course}</td><td>${b.time}</td><td>${b.topic}</td></tr>`).join('')}</tbody></table>
                </div>
            </body>
            </html>
        `;
        ...
        const pdfUint8Array = yield page.pdf({ format: 'A4', printBackground: true });
        const pdfBuffer = Buffer.from(pdfUint8Array);
        yield browser.close();
        return pdfBuffer;
    }),
    generateAdminReportPDF: (pmName) => __awaiter(void 0, void 0, void 0, function* () {
